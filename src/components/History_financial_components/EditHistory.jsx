...
function EditHistory({ data, table, closeModal }) {
  const { dispatch } = useContext(HomeChangeContext);
  ...
  const sendInfo = async (e) => {
    ...
    const response = await fetchDataPut(
      `/api/v1/users/financial/${table}/edit/`,
      body
    );
    ...
    toast((t) => (
      <span className="message_toas">
        {response.message ? (
          <div className="toas_message_ok">
            {" "}
            <IoMdCheckmarkCircleOutline />
            {response.message}
          </div>
        ) : (
          <div className="toas_message_error">
            <IoIosCloseCircle />
            <> Ah ocurrido un error, intentelo de nuevo más tarde.</>
          </div>
        )}
  ...
  return (
    <>
      ...
      <div
        className={`${
          file ? "flex" : "hidden"
        } justify-center items-center w-[100%] max-w-[559px] h-fit md:h-[604px] rounded-[10px] pt-[45px] pb-[45px] relative bg-white`}
      >
        {file && (
          <img src={file} alt="" className="object-cover w-[70%] rounded-lg" />
        )}
      </div>
    </>
  );
